...
void GPIO_HSPI_INIT(void)
{
	...
	#if	HSPI_OFF_WITH_VCC			// leox_20120112 [PATCH]
	SET_MASK(GCR3, HSPIPDG);
	#endif
}
...
void HSPI_CmdRamCode(void)
{
	...
	while (1)
	{
		...
		if (RamcodeCmd == 0x55)
		{
			...
			#if	0	// 120731 chris add for fix flash EC by WINIFU then shutdown then boot fail [PATCH]
			RstLaterMark0 = 4;
			RstLaterMark1 = 3;
			#endif
			WinFlashMark = 0x33;
		...
		else if (RamcodeCmd == 0xAF)
		{
			...
			#if	0	// leox_20111214 [PATCH] Disabled to prevent code excess 256 bytes
			//reboot for IFU.exe tool ++	// leox_20111206 [PATCH]
