/*------------------------------------------------------------------------------
 * Title: CORE_SCAN.C - Code to handle local keyboard scanning.
 *
...
{
	//David_20120510 add for reset extend keyboard matrix
	if (ExtendMatrix)
	{
		KSOH2 = 0xFF;
...
void service_scan(void)
{
	...
	else							// No scan activity
	{
		...
		FN_Key_Break_HS = 0x00;
		table_entry_bk = 0x00;
		ClearFnKeyBundledFlag();	// @JayFn [PATCH]
	}
...
BYTE bScanExtendKeys(void)
{
	...
	if (ITempB06 == FALSE)
	{
		for (ITempB01 = 0; ITempB01 < ExtendScanPin; ITempB01++)
		{
			...
			//David_20120327 add for extend keyboard matrix scan
			CapDelay();
			//David_20120327 add for extend keyboard matrix scan
			ITempB02 = KSI;
	...
	#if	0	//David_20120510 add for reset extend keyboard matrix
	if (extendstatus.field.ghost == 1)
	{
		extendstatus.field.ghost = 0;	// Ghost flag is set.
		ITempB01 = extendstatus.byte & 7;
	...
	#endif	//David_20120510 add for reset extend keyboard matrix
